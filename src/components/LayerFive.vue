...
<script>
    ...
    export default {
                ...
                for(var i=0; i<data_num; i++){
                    for(var j=0; j<data_num; j++){
                        if(resp.data[i].itemName == this.exact[j].itemName){
                            ...
                            if(index!=-1)
                                this.layer5.splice(index,1)
                            this.layer5.push(temp)

                            this.$emit("layer_insert",resp.data[i].itemName, resp.data[i].quantity, 5, resp.data[i].remark)
        ...
        methods: {
            editItem (item) {
                this.editedIndex = this.layer5.indexOf(item)
            ...
            save () {
                if (this.editedIndex > -1) {
                    ...
                    Object.assign(this.layer5[this.editedIndex], this.editedItem)
                    this.$emit("layer_change", this.editedItem.name, temp, this.editedItem.quantity, this.editedItem.remark, 5)
                } else {
                    this.layer5.push(this.editedItem)
                }
                this.close()
            },
        }
